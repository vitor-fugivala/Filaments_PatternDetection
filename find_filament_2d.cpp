...
void write_output(vector<Vector2d> &points, string suffix, bool verbose){
    stringstream ss;
    ss << suffix;
    if (verbose) {
...
int main(int argc, char* argv[]){
    ...
    float h,t,epsilon;
    while ((opt = getopt(argc, argv, "d:h:i:t:e:r:n:q")) != -1){
        switch(opt){
    ...
    write_output(thresholded_points, out_filename, verbose);
